        ...
        for stage in range(len(args.epoch_setting)):
            ...
            if args.warmup_stage > -1:
                if stage <= args.warmup_stage:
                    probs_path = os.path.join(args.probs_dir, 
                                              ...
                                              f'use_labels_{args.use_labels}_use_feats_{not args.avoid_features}_K_{args.K}_label_K_{args.label_K}_probs_seed_{args.seed + i}_stage_{stage}_pretrain_{str(not args.pretrain_path=='None')}.pt')
                    print(probs_path)
            ...
            if stage > 0:
                probs_path = os.path.join(args.probs_dir, args.dataset, 
                                args.model if (args.weight_style == "attention") else (args.model + "_" + args.weight_style),
                                f'use_labels_{args.use_labels}_use_feats_{not args.avoid_features}_K_{args.K}_label_K_{args.label_K}_probs_seed_{args.seed + i}_stage_{stage - 1}_pretrain_{str(not args.pretrain_path=='None')}.pt')
            else:
                probs_path = ''
            ...
            path = os.path.join("../converge_stats", args.dataset, 
                                args.model if (args.weight_style == "attention") else (args.model + "_" + args.weight_style),
                                f"use_labels_{args.use_labels}_use_feats_{not args.avoid_features}_K_{args.K}_label_K_{args.label_K}_seed_{args.seed + i}_stage_{stage}_pretrain_{str(not args.pretrain_path=='None')}.csv")
            ...
            fig_path = os.path.join("../converge_stats", args.dataset, 
                        args.model if (args.weight_style == "attention") else (args.model + "_" + args.weight_style),
                        f"use_labels_{args.use_labels}_use_feats_{not args.avoid_features}_K_{args.K}_label_K_{args.label_K}_seed_{args.seed + i}_stage_{stage}_pretrain_{str(not args.pretrain_path=='None')}.png")
            sns.set()
            ...
            path = os.path.join("../converge_stats", args.dataset, 
                                args.model if (args.weight_style == "attention") else (args.model + "_" + args.weight_style),
                                f"val_loss_use_labels_{args.use_labels}_use_feats_{not args.avoid_features}_K_{args.K}_label_K_{args.label_K}_seed_{args.seed + i}_stage_{stage}_pretrain_{str(not args.pretrain_path=='None')}.csv")
            if not os.path.exists(os.path.dirname(path)):
                os.makedirs(os.path.dirname(path))
            # print(val_loss)
            df = pd.DataFrame()
            ...
            fig_path = os.path.join("../converge_stats", args.dataset, 
                        args.model if (args.weight_style == "attention") else (args.model + "_" + args.weight_style),
                        f"val_loss_use_labels_{args.use_labels}_use_feats_{not args.avoid_features}_K_{args.K}_label_K_{args.label_K}_seed_{args.seed + i}_stage_{stage}_pretrain_{str(not args.pretrain_path=='None')}.png")
            sns.set()
